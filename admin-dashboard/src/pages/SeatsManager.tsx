...
export default function SeatsManager() {
  ...
  const toggleSeatStatus = async (seat: Seat) => {
    ...
    try {
      ...
      setSeats(
        seats.map((s) =>
          s._id === seat._id ? { ...s, status: nextStatus } : s,
        ),
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col p-8 bg-card border border-card-border rounded-3xl">
        ...
        <p className="text-subtext mb-6">
          Bulk generate and monitor seating capacities per section.
        </p>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <select
              value={filterVenueId}
              onChange={(e) => {
                setFilterVenueId(e.target.value);
            ...
            >
              ...
              {validSections.map((s) => (
                <option key={s._id} value={s._id}>
                  {s.name} ({s.venue_id?.name || "Unknown Venue"})
                </option>
              ))}
            </select>
          </div>
      ...
      {filterSectionId === "All" ? (
        <div className="py-20 text-center border border-dashed border-card-border rounded-3xl bg-card/50">
          ...
          <p className="text-subtext">
            You must select a specific section to manage its seats.
          </p>
        </div>
      ) : (
            ...
            <div className="overflow-x-auto">
              {loadingSeats ? (
                ...
                <div className="py-12 text-center text-subtext">
                  No seats generated for this section yet.
                </div>
              ) : (
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-card-border/30 text-subtext text-xs uppercase tracking-wider">
                      ...
                      <th className="p-4 font-bold text-right rounded-tr-3xl">
                        Actions
                      </th>
                    </tr>
                  </thead>
            ...
            {totalSeats > 0 && (
              <div className="flex items-center justify-between p-4 border-t border-card-border bg-background/50">
                <span className="text-sm font-medium text-subtext">
                  ...
                  {Math.min(page * limit, totalSeats)} of {totalSeats} seats
                </span>
      ...
      {generatorOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-sm rounded-4xl border border-card-border shadow-2xl p-6">
            <h3 className="text-xl font-bold text-text mb-2">
              Automated Generator
            </h3>
            <p className="text-sm text-subtext mb-6">
              Create blocked sequential seats instantly.
            </p>

            <form onSubmit={handleGenerate} className="space-y-4">
              {genError && (
                <div className="text-xs text-error font-bold p-2 bg-error/10 rounded-lg">
                  {genError}
                </div>
              )}
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block text-xs font-bold text-subtext mb-1 uppercase tracking-wider">
                    Start Number
                  </label>
                  <input
                    type="number"
                    ...
                    className="w-full bg-background border border-card-border rounded-xl p-3 text-text font-mono"
                  />
                </div>
                <div className="flex-1">
                  <label className="block text-xs font-bold text-subtext mb-1 uppercase tracking-wider">
              ...
              <div className="pt-4 flex gap-3">
                <button
                  ...
                  className="flex-1 py-3 text-subtext font-bold rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
                  disabled={generating}
                  type="submit"
                  className="flex-1 bg-primary text-white font-bold rounded-xl transition-colors disabled:opacity-50"
                >
                  {generating ? (
      ...
      {wipeConfirmOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
          <div className="bg-card w-full max-w-sm rounded-4xl border border-error/50 shadow-2xl shadow-error/10 p-6 text-center">
            ...
            <p className="text-sm text-subtext mb-6">
              This action is irreversible and drops all current seating capacity
              for the chosen section.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setWipeConfirmOpen(false)}
                className="flex-1 py-3 bg-background border border-card-border rounded-xl font-bold text-text"
              >
                Cancel
              </button>
              <button
                onClick={handleWipe}
                disabled={wiping}
                className="flex-1 py-3 bg-error text-white font-bold rounded-xl disabled:opacity-50"
              >
                {wiping ? (
                  <Loader2 className="animate-spin mx-auto" />
                ) : (
