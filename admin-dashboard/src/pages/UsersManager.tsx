...
import { Loader2, Search, Users, Shield, Calendar, Phone, Mail } from 'lucide-react';
...
export default function UsersManager() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await fetch(`http://localhost:5001/api/users/all?${queryParams}`, {
          headers: {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 p-8 bg-card border border-card-border rounded-3xl">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-text mb-2">Users Management</h2>
          <p className="text-subtext">View registered attendees and system administrators.</p>
        </div>

        <form onSubmit={handleSearch} className="relative w-full md:w-80">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-subtext" size={20} />
            <input 
              type="text" 
      ...
      {loading && users.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center text-primary">
              ...
              <p className="mt-4 font-medium text-subtext animate-pulse">Loading users...</p>
          </div>
      ) : (
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {users.map(u => (
                    <div key={u._id} className="bg-card border border-card-border rounded-3xl p-6 shadow-sm hover:shadow-lg transition-shadow overflow-hidden relative group">
                        ...
                        <div className="flex items-center gap-4 mb-6">
                            {u.profile_photo ? (
                                <img src={u.profile_photo} alt={u.name} className="w-16 h-16 rounded-2xl object-cover border border-card-border bg-background" />
                            ) : (
                            ...
                            <div>
                                <h3 className="font-bold text-lg text-text leading-tight truncate max-w-[180px]">{u.name}</h3>
                                <div className="flex items-center gap-1.5 text-xs font-semibold mt-1">
                                    {u.is_verified ? (
                                        <span className="text-success bg-success/10 px-2 py-0.5 rounded-md">Verified</span>
                                    ) : (
                                        <span className="text-subtext bg-card-border/50 px-2 py-0.5 rounded-md">Unverified</span>
                                    )}
                                </div>
                            </div>
                        </div>
            ...
            {users.length === 0 && !loading && (
                <div className="py-20 text-center border-2 border-dashed border-card-border rounded-3xl bg-card">
                    ...
                    <h3 className="text-lg font-bold text-text mb-1">No users found</h3>
