...
import { Loader2, Ticket as TicketIcon, Filter, Search, ChevronLeft, ChevronRight, User as UserIcon } from 'lucide-react';
...
export default function TicketsManager() {
  ...
  const getStatusBadge = (status: string) => {
      switch (status) {
          case 'Active': return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-primary/10 text-primary border border-primary/20">Active</span>;
          case 'Used': return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-success/10 text-success border border-success/20">Used</span>;
          case 'Refunded': return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-error/10 text-error border border-error/20">Refunded</span>;
          default: return <span className="px-2.5 py-1 text-xs font-bold rounded-lg bg-card-border/50 text-subtext">Unknown</span>;
      }
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-8 bg-card border border-card-border rounded-3xl">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-text mb-2">Box Office Tracker</h2>
          <p className="text-subtext">Monitor and filter globally issued tickets in real-time.</p>
        </div>
              ...
              {loading && tickets.length === 0 ? (
                  ...
                  <table className="w-full text-left border-collapse">
                      ...
                      <tbody className="divide-y divide-card-border">
                          {tickets.map(ticket => (
                              <tr key={ticket._id} className="hover:bg-card-border/10 transition-colors group">
                                  {/* Hash */}
                                  <td className="p-5">
                                      ...
                                      {ticket.event_id ? (
                                        <div className="flex items-center gap-3">
                                            {ticket.event_id.imageUrl ? (
                                                <img src={ticket.event_id.imageUrl} className="w-10 h-10 rounded-lg object-cover bg-card-border shrink-0" />
                                            ) : (
                                                <div className="w-10 h-10 rounded-lg bg-card-border/50 shrink-0" />
                                            )}
                                            <div>
                                                <div className="font-bold text-text text-sm truncate max-w-[150px]">{ticket.event_id.title}</div>
                                                <div className="text-xs text-subtext">{new Date(ticket.event_id.date).toLocaleDateString()}</div>
                                            </div>
                                        </div>
                                      ) : <span className="text-subtext italic text-sm">Unknown</span>}
                                  </td>
                                  ...
                                  <td className="p-5">
                                    {ticket.attendee_id ? (
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <div className="font-bold text-text text-sm truncate max-w-[120px]">{ticket.attendee_id.name}</div>
                                                <div className="text-xs text-subtext truncate max-w-[120px]">{ticket.attendee_id.email}</div>
                                            </div>
                                        </div>
                                    ) : <span className="text-subtext italic text-sm">Guest</span>}
                                  </td>
                                  ...
                                  <td className="p-5">
                                    {ticket.seat_id ? (
                                        <div className="flex flex-col">
                                            <span className="font-bold text-text text-sm">Seat {ticket.seat_id.number}</span>
                                            <span className="text-xs text-primary font-bold">{ticket.seat_id.section_id?.name || 'Gen Ad'}</span>
                                        </div>
          ...
          {totalTickets > 0 && (
              <div className="flex items-center justify-between p-4 border-t border-card-border bg-background/50">
                  <span className="text-sm font-medium text-subtext">
                      Showing {(page - 1) * limit + 1} to {Math.min(page * limit, totalTickets)} of {totalTickets} metrics
                  </span>
