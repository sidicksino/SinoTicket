  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-background gap-4">
        ...
        <p className="text-subtext font-medium animate-pulse">{t('auth.verifyingAccess')}</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen w-full flex bg-background items-center justify-center p-8">
        <div className="bg-card p-8 rounded-4xl border border-error/20 shadow-2xl shadow-error/10 text-center max-w-sm w-full">
          ...
          <h1 className="text-2xl font-bold text-error mb-2">{t('auth.accessDenied')}</h1>
          ...
          <div className="flex items-center justify-center gap-2 mb-4">
            ...
            <p className="text-subtext text-xs">{t('auth.redirectingToLogin')}</p>
          </div>
          <button
            onClick={() => signOut()}
